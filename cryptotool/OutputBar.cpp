...
void COutputBar::ClearTabs()
{
	...
	*/
	//GetTabsNum是获得AddTab添加的对象，添加的CEdit我现在不用auto_ptr不能删除了
	m_wndTabs.RemoveAllTabs();
}

void COutputBar::AddTab(LPCTSTR lpszName)
{
	//std::auto_ptr<CEdit> apWndEdit(new CEdit());
	m_wndEdit.Create(WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL |
		ES_MULTILINE | ES_AUTOHSCROLL | ES_AUTOVSCROLL,
		CRect(0, 0, 0, 0), &m_wndTabs, 1);
	m_wndEdit.SetReadOnly();
	m_wndEdit.SetFont(m_Font.GetSafeHandle() == NULL ? &afxGlobalData.fontRegular : &m_Font);

	m_wndTabs.AddTab(&m_wndEdit, lpszName, (UINT)-1, FALSE);
}

void COutputBar::InsertLog(LPCTSTR lpszLog)
{
	CString strLog, strLineEnd;
	m_wndEdit.GetWindowText(strLog);
	strLineEnd.Format("\r\n");
	strLog = strLog + lpszLog + strLineEnd;
	m_wndEdit.SetWindowTextA(strLog);
}
